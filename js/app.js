...
(function() {
  ...
  function t(key, replacements = {}) {
    const langDict = window.i18n.be || window.i18n[state.currentLang] || window.i18n.ru;
    let str = (langDict && langDict[key]) || (window.i18n.be && window.i18n.be[key]) || (window.i18n.ru && window.i18n.ru[key]) || key;
  ...
  function updateInterfaceLanguage() {
    ...
    if (dict1 && elements.dictTab1Label) {
      elements.dictTab1Label.textContent = dict1.nameBe || dict1.nameRu;
      if (elements.dictTab1ShortLabel) {
        elements.dictTab1ShortLabel.textContent = dict1.shortNameBe || dict1.nameBe;
      }
    }
    if (dict2 && elements.dictTab2Label) {
      elements.dictTab2Label.textContent = dict2.nameBe || dict2.nameRu;
      if (elements.dictTab2ShortLabel) {
        elements.dictTab2ShortLabel.textContent = dict2.shortNameBe || dict2.nameBe;
      }
    }
  ...
  function updateDictionaryHeaderInfo() {
    ...
    const dictName = dict.nameBe || dict.nameRu;
  ...
  function createWordCard(item, query) {
    ...
    const authorIcon = state.currentDictId === 'dict1' ? '❄️' : '🌿';
  ...
  function openWordModal(item) {
    ...
    const authorEmblem = theme?.emblemImg || (state.currentDictId === 'dict1' ? 'assets/images/nekrasov_emblem.jpg' : 'assets/images/kolas_emblem.jpg');
    const authorName = theme?.authorName || '';
    const authorIcon = theme?.icon || (state.currentDictId === 'dict1' ? '❄️' : '🌿');
  ...
  function downloadDictionariesJs() {
    ...
    showToast(t('exportJsDesc') || 'Файл dictionaries.js спампаваны!');
  }
  ...
  function openEditWordModal(item = null) {
    ...
    if (item) {
      ...
      elements.editWordWork.value = state.currentDictId === 'dict1' ? '«Мароз, Чырвоны нос»' : 'Трылогія «На ростанях»';
      elements.editWordSuffix.value = '-к-';
      elements.editWordGrammar.value = 'назоўнік, ж.р., 1-е скл., Н. скл.';
      elements.editWordMeaning.value = '';
      elements.editWordDerivation.value = '';
      elements.editWordTags.value = '';
      elements.editWordStylistics.value = 'нейтральнае';
    }
  ...
  function handleAddWordSubmit(e) {
    ...
    const newEntry = {
      ...
      word,
      stressed,
      work: state.currentDictId === 'dict1' ? '«Мароз, Чырвоны нос»' : 'Трылогія «На ростанях»',
      workShort: state.currentDictId === 'dict1' ? '«Мароз, Чырвоны нос»' : '«На ростанях»',
      pos,
      gender,
      grammar: gender ? `назоўнік, ${gender}` : 'назоўнік',
      meaning: translation,
      translation,
      examples,
      synonyms,
      tags: ['дададзена адміністратарам']
    };
    ...
    showToast('Слова паспяхова захавана!');
    closeDataModal();
  }
  ...
  function handleApplyImport() {
    ...
    try {
      ...
      const formatted = parsed.map((item, idx) => ({
        ...
        work: item.work || (state.currentDictId === 'dict1' ? '«Мароз, Чырвоны нос»' : 'Трылогія «На ростанях»'),
        grammar: item.grammar || 'назоўнік, ж.р.',
